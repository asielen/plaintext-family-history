...
RENAME_PID = 'P-rm6wr4t70q'
RENAME_FOLDER = '040 Thomas Kelly + Betty Turley Kelly [Mary]'
RENAME_NEW_FOLDER = '040 Thomas Kelly + Betty Slamon Kelly [Mary]'


def _rename_profile_text(name: str, *, variants: str | None = None) -> str:
    variants_line = f'name_variants: [{variants}]\n' if variants else ''
    return (
        '---\n'
        f'id: {RENAME_PID}\n'
        f'name: {name}\n'
...
_RENAME_RESEARCH_TEXT = (
    f'---\nid: {RENAME_PID}\ncreated: 2026-01-01\n---\n\n'
    '## Research Notes\nSome hand-written notes that must survive.\n'
)

_RENAME_GENERATED_TIMELINE = (
    '<!-- GENERATED by fha views timeline - do not edit by hand -->\n'
    '# Timeline: Betty Turley Kelly\n'
)


class RenameTests(unittest.TestCase):
    """`fha person rename` - issue #51's name-correction verb.

    Covers the full couple-folder case (frontmatter name/name_variants, the
    profile and research file renames, stale GENERATED companion cleanup,
    and the couple-folder text rename), a person who is NOT half of a couple
    (no folder step at all), --dry-run zero-writes, the already-named no-op,
    the filename-collision refusal, a non-GENERATED look-alike companion
    left untouched, an ambiguous/absent folder-name match left alone, and
    the Windows cwd-inside-the-folder guard (issue #51's own reproduction).
    """

    def setUp(self) -> None:
        self._tmp = tempfile.TemporaryDirectory()
        self.root = Path(self._tmp.name)
        self.folder = self.root / 'people' / RENAME_FOLDER
        self.folder.mkdir(parents=True)
        self.profile = self.folder / f'kelly__betty_turley_{RENAME_PID}.md'
        self.profile.write_text(
            _rename_profile_text('Betty Turley Kelly', variants='Betty T. Kelly'),
            encoding='utf-8')
        self.research = self.folder / f'kelly__betty_turley_research_{RENAME_PID}.md'
        self.research.write_text(_RENAME_RESEARCH_TEXT, encoding='utf-8')
        self.timeline = self.folder / f'kelly__betty_turley_timeline_{RENAME_PID}.md'
        self.timeline.write_text(_RENAME_GENERATED_TIMELINE, encoding='utf-8')
    ...
    def _new_paths(self) -> tuple[Path, Path]:
        # stub_filename embeds the LOWERCASE-normalized id (the same
        # convention `fha person new` already uses - see run_new's own
        # `pid = mint_ids(...)[0].lower()`), so the filename's id suffix is
        # lowercase even though the record's own id: field stays display-cased.
        new_folder = self.root / 'people' / RENAME_NEW_FOLDER
        pid_lower = RENAME_PID.lower()
        return (new_folder / f'kelly__betty_slamon_{pid_lower}.md',
                new_folder / f'kelly__betty_slamon_research_{pid_lower}.md')

    def test_happy_path_renames_everything(self) -> None:
        ...
        self.assertTrue(new_profile.exists(), list(self.root.rglob('*')))
        self.assertTrue(new_research.exists())
        self.assertFalse(self.profile.exists())
        self.assertFalse(self.research.exists())
        self.assertFalse(self.timeline.exists())   # stale GENERATED companion gone

        text = new_profile.read_text(encoding='utf-8')
        self.assertIn('name: Betty Slamon Kelly', text)
        # The OLD name was filed into name_variants: alongside the existing one.
        self.assertIn('Betty T. Kelly', text)
        self.assertIn('Betty Turley Kelly', text)
        self.assertIn(f'id: {RENAME_PID}', text)   # id: untouched

        # The research file's hand-written notes travelled unchanged.
        self.assertIn('Some hand-written notes that must survive.',
                      new_research.read_text(encoding='utf-8'))

        # The couple folder itself was renamed to match.
        self.assertFalse(self.folder.exists())
        self.assertTrue((self.root / 'people' / RENAME_NEW_FOLDER).is_dir())
        self.assertEqual(res.data['folder'], RENAME_FOLDER)
        self.assertEqual(res.data['new_folder'], RENAME_NEW_FOLDER)

        all_text = ' '.join(m.text for m in res.messages)
        self.assertIn('fha views timeline', all_text)   # regeneration next-step

    def test_dry_run_writes_nothing(self) -> None:
        before = {p: p.read_bytes() for p in self.root.rglob('*') if p.is_file()}
        res = person.run_rename(self.root, RENAME_PID, 'Betty Slamon Kelly',
                                keep_variant=True, dry_run=True)
        self.assertEqual(res.exit_code, EXIT_CLEAN)
        self.assertEqual(res.data['status'], 'dry-run')
        after = {p: p.read_bytes() for p in self.root.rglob('*') if p.is_file()}
        self.assertEqual(before, after)
        self.assertTrue(self.folder.is_dir())   # old folder name untouched
        all_text = ' '.join(m.text for m in res.messages)
        self.assertIn('Betty Slamon Kelly', all_text)
        self.assertIn(RENAME_NEW_FOLDER, all_text)

    def test_without_keep_variant_old_name_is_dropped_not_filed(self) -> None:
        res = person.run_rename(self.root, RENAME_PID, 'Betty Slamon Kelly')
        self.assertEqual(res.exit_code, EXIT_CLEAN, res.messages)
        new_profile, _ = self._new_paths()
        text = new_profile.read_text(encoding='utf-8')
        self.assertIn('name: Betty Slamon Kelly', text)
        # The existing variant survives; the old NAME is never auto-added
        # without --keep-variant.
        self.assertIn('Betty T. Kelly', text)
    ...
    def test_person_not_half_of_a_couple_skips_the_folder_step(self) -> None:
        # A stub, alone in people/stubs/ - not a couple-folder occupant at all.
        stubs = self.root / 'people' / 'stubs'
        stubs.mkdir(parents=True)
        solo_pid = 'P-s01ag3ntv0'
        profile = stubs / f'doe__jane_{solo_pid}.md'
        profile.write_text(
            f'---\nid: {solo_pid}\nname: Jane Doe\nliving: unknown\ntier: stub\n'
            'created: 2026-01-01\n---\n', encoding='utf-8')
        res = person.run_rename(self.root, solo_pid, 'Jane Smith')
        self.assertEqual(res.exit_code, EXIT_CLEAN, res.messages)
        self.assertEqual(res.data['status'], 'ok')
    ...
    def test_filename_collision_refuses_with_nothing_written(self) -> None:
        # Another file already sits at the exact filename the new name would
        # derive - refuse rather than overwrite it. Built as its own isolated
        # fixture (not the shared setUp state) so the two filenames sort with
        # the REAL profile alphabetically FIRST ('aardvark' < 'zzyx') -
        # `find_person_record_path`'s scan returns the first non-companion-
        # looking match for an id (`_lib.py`'s documented, pre-existing
        # behavior for two files that share an id suffix), so this ordering
        # is what makes the fixture locate the real record rather than the
        # obstacle file sitting at the destination path.
        pid = 'P-c9a2b3d4e5'
        folder = self.root / 'people' / 'connections'
        folder.mkdir(parents=True)
        real = folder / f'kelly__betty_aardvark_{pid}.md'
        real.write_text(
            f'---\nid: {pid}\nname: Betty Aardvark Kelly\nliving: false\n'
            'tier: curated\ncreated: 2026-01-01\n---\n', encoding='utf-8')
        obstacle = folder / f'kelly__betty_zzyx_{pid.lower()}.md'
        obstacle.write_text('---\nid: P-zzzzzzzzzz\nname: Someone Else\n---\n',
                            encoding='utf-8')
        before = real.read_bytes()
        res = person.run_rename(self.root, pid, 'Betty Zzyx Kelly')
        self.assertEqual(res.exit_code, EXIT_FAILURE)
        self.assertEqual(res.data['status'], 'refused')
        self.assertTrue(real.exists())
        self.assertEqual(real.read_bytes(), before)   # untouched
        self.assertIn('already exists', res.messages[-1].text)

    def test_non_generated_lookalike_companion_is_left_alone(self) -> None:
        # A file sharing the person's ID suffix that carries NO GENERATED
        # header must never be deleted, even though it looks like a stale
        # companion by name.
        hand_file = self.folder / f'kelly__betty_turley_photos_{RENAME_PID}.md'
        hand_file.write_text('# Hand-curated photo notes\nNot generated.\n', encoding='utf-8')
        res = person.run_rename(self.root, RENAME_PID, 'Betty Slamon Kelly')
        self.assertEqual(res.exit_code, EXIT_CLEAN, res.messages)
        # It moved with the folder rename but was never deleted.
        moved = self.root / 'people' / RENAME_NEW_FOLDER / hand_file.name
        self.assertTrue(moved.exists())
        self.assertIn('Not generated.', moved.read_text(encoding='utf-8'))
        all_text = ' '.join(m.text for m in res.messages)
        self.assertIn(hand_file.name, all_text)   # named, not silently skipped

    def test_folder_left_alone_when_old_name_not_in_its_text(self) -> None:
        # Rename the folder itself to a nickname-based name that does not
        # contain "Betty Turley Kelly" verbatim - the rename must not guess.
        odd_folder = self.root / 'people' / '040 Thomas Kelly + Betty [Mary]'
        self.folder.rename(odd_folder)
        profile = odd_folder / f'kelly__betty_turley_{RENAME_PID}.md'
        res = person.run_rename(self.root, RENAME_PID, 'Betty Slamon Kelly')
        self.assertEqual(res.exit_code, EXIT_CLEAN, res.messages)
        self.assertIsNone(res.data['new_folder'])
    ...
    def test_cwd_inside_folder_blocks_only_the_folder_step(self) -> None:
        # Issue #51's exact reproduction: the shell's own working directory
        # sits inside the folder being renamed. The record/file renames must
        # still land; only the folder rename is refused, with a plain-language
        # cause and an exact retry - never a raw PermissionError/OSError.
        old_cwd = os.getcwd()
        ...
        try:
            ...
            os.chdir(old_cwd)
        self.assertEqual(res.exit_code, EXIT_WARNINGS)
        self.assertEqual(res.data['status'], 'ok-folder-unrenamed')
        ...
        warning_text = ' '.join(m.text for m in res.messages if m.level == 'warning')
        self.assertIn('current folder is inside', warning_text)
        self.assertIn('fha person rename', warning_text)
    ...
    def test_cli_dry_run_via_fha(self) -> None:
        ...
        out = io.StringIO()
        with contextlib.redirect_stdout(out), contextlib.redirect_stderr(io.StringIO()):
            rc = fha.main(['person', 'rename', RENAME_PID, 'Betty Slamon Kelly',
                           '--dry-run', '--root', str(self.root)])
        self.assertEqual(rc, 0)
        self.assertIn('[dry-run]', out.getvalue())
        self.assertTrue(self.profile.exists())

    def test_cli_live_write(self) -> None:
        import fha
        out = io.StringIO()
        with contextlib.redirect_stdout(out), contextlib.redirect_stderr(io.StringIO()):
            rc = fha.main(['person', 'rename', RENAME_PID, 'Betty Slamon Kelly',
                           '--keep-variant', '--root', str(self.root)])
        self.assertEqual(rc, 0)
        new_profile, _ = self._new_paths()
        self.assertTrue(new_profile.exists())

    def test_unknown_id_is_not_found_with_next_step(self) -> None:
        res = person.run_rename(self.root, 'P-0000000000', 'Nobody')
        self.assertEqual(res.exit_code, EXIT_WARNINGS)
        self.assertEqual(res.data['status'], 'not-found')

    def test_merged_tombstone_refuses_naming_survivor(self) -> None:
        survivor = 'P-de957bcda1'
        self.profile.write_text(
            f'---\nid: {RENAME_PID}\nname: Betty Turley Kelly\nliving: false\n'
            f'status: merged\nmerged_into: {survivor}\n---\n', encoding='utf-8')
        res = person.run_rename(self.root, RENAME_PID, 'Betty Slamon Kelly')
        self.assertEqual(res.exit_code, EXIT_FAILURE)
        self.assertEqual(res.data['status'], 'merged')
        self.assertIn(survivor, res.messages[-1].text)
