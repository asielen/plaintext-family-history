...
class _ViewsHtmlBase(unittest.TestCase):
    ...
    def _runners(self):
        # #76: per-person sources-index no longer has an html twin at all (it
        # rewrites the profile's own ## Sources region, not a separate file),
        # so it is EXCLUDED from this shared md|html matrix helper - see
        # SourcesIndexHtmlTests below for its own, differently-shaped coverage
        # (the refusal, and the still-unaffected couple-folder html form).
        return (
...
class SourcesIndexHtmlTests(_ViewsHtmlBase):
    """#76: per-person sources-index has no html twin (it is an inline
    profile region now); the couple-folder form is unaffected and still has
    one. Kept separate from `_runners()`/HtmlFormatMatrixTests/
    HtmlContentTests, which now cover only timeline/draft-queue - see the
    comment on `_runners()` for why."""

    def test_per_person_html_is_refused(self):
        res = views.run_sources_index(self.root, person_id=PID, fmt='html')
        self.assertEqual(res.exit_code, EXIT_FAILURE)
        self.assertFalse(res.changed)

    def test_all_curated_html_without_couple_folders_is_refused(self):
        # Nothing left to build for --all-curated --format html on its own
        # (per-person has no html; --couple-folders is what still does).
        res = views.run_sources_index(self.root, all_curated=True, fmt='html')
        self.assertEqual(res.exit_code, EXIT_FAILURE)
        self.assertFalse(res.changed)

    def test_couple_folder_html_is_unaffected(self):
        res = views.run_sources_index(self.root, couple_folders_only=True, fmt='html')
        self.assertEqual(res.exit_code, EXIT_CLEAN)
        self.assertEqual(len(res.changed), 1)
        out = Path(res.changed[0])
        self.assertEqual(out.parent, self._gen_dir())
        self.assertTrue(out.name.startswith(COUPLE_DIR), out.name)
        text = out.read_text(encoding='utf-8')
        self.assertTrue(text.splitlines()[0].startswith(
            '<!-- GENERATED by fha views sources-index'))

    def test_all_curated_with_couple_folders_html_builds_only_the_folder_form(self):
        res = views.run_sources_index(
            self.root, all_curated=True, couple_folders_only=True, fmt='html')
        self.assertEqual(res.exit_code, EXIT_CLEAN)
...
class CleanSweepTests(_ViewsHtmlBase):
    def test_clean_sweeps_generated_views_marker_per_file(self):
        ...
        self.assertEqual(sorted(p.name for p in self._gen_dir().iterdir()),
                         gen_before)
        # people/-tree .md + generated/views/ html were both counted. #76: only
        # 2 per-person companions now (timeline, draft-queue) - sources-index
        # is an inline profile region, never a companion `views clean` sweeps -
        # plus the couple-folder sources-index.md.
        self.assertEqual(res.data.get('would_remove'),
                         len(gen_before) - 1 + 3)  # 2 companions + folder index
...
class RefreshFormatTests(_ViewsHtmlBase):
    def test_refresh_both_writes_both_sets(self):
        ...
        self.assertEqual(res.exit_code, EXIT_CLEAN)
        # Per person: timeline + draft-queue, 2 md + 2 html; the person's own
        # ## Sources region refreshed once (md-only - #76, no html twin);
        # couple folder index: 1 md + 1 html.
        self.assertEqual(res.data.get('count'), 7)
        ...
        self.assertIn('## Sources', self.profile.read_text(encoding='utf-8'))

    def test_refresh_html_writes_only_html(self):
        res = views.run_refresh(self.root, fmt='html')
        self.assertEqual(res.exit_code, EXIT_CLEAN)
        # Per person: timeline + draft-queue html only (no per-person sources-
        # index write at all on an html-only pass - #76); couple folder: 1 html.
        self.assertEqual(res.data.get('count'), 3)
        self.assertFalse(
            (self.profile.parent / f'hartley__cur_view_timeline_{PID}.md').exists())
        self.assertFalse((self.profile.parent / 'sources-index.md').exists())
        # And the profile's own body is untouched by an html-only refresh.
        self.assertNotIn('## Sources', self.profile.read_text(encoding='utf-8'))
        self.assertEqual(len(list(self._gen_dir().iterdir())), 3)
