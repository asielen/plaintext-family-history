...
A clipping that names no one at all is a different case from any of the three above: file it as `source_type: ephemera` (SPEC §14) with `people:` left empty rather than linked to a "who it's about" that doesn't exist. Because nothing links it to a person, it will *not* show up on anyone's source list, site page, or packet - those are all found by following a person's links. It's still kept and still findable: it gets its own page and shows up in the generated site's homepage Sources list (grouped by decade, with an "Undated" group for open dates), and its notes are searched like any other. If a clipping turns out to name someone after all, even in passing, it isn't ephemera - retype it and link the person, and it becomes case 2 above.
