...
5. **Record the AI pass** in the source(s) you drew from, or note the drafting pass on the person's
   research file, per `## AI Passes` shape ({date, model, harness, task, outputs, human_reviewed}).
