...
*One exception exists so install-once does not mean install-once-then-lost:* when a **layout change moves a skeleton seed's archive path** (`design/custom.css` -> `.fha/design/custom.css`), `update-tools` **moves the human's file to the new path first**, carrying the customization with it, and re-keys the stamp. Without that step the normal reconciliation would read the old path as retired (the manifest no longer lists it), quarantine it into `.plaintext-backup/`, and install nothing in its place - because updates never install skeleton entries - so a customized stylesheet would vanish from the archive. The move is reported like any other change, is matched only where the manifest lists the vendor-prefixed twin as a skeleton entry and the destination does not already exist, and is previewed by `--dry-run`.
