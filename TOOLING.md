...
**`fha photoindex set-summary (<path> | --group <group-id>) --text "…" [--append] [--dry-run]`** writes a photo's embedded AI summary (`UserComment`) via exiftool - the caption-improvement write SPEC §20 rule 5 already sanctions. `<path>` writes one catalog file; `--group` writes every member of the variation group, so fronts/backs/copies stay consistent. The compose rule fails toward preservation: new text is always written AI-marked (`AI: <text>`, the `^(AI|Model):` convention triage already recognizes); an existing AI-marked comment is replaced (`--append` keeps it and adds the new block below); an existing unmarked comment is human text - preserved verbatim with the AI block appended below, flag or no flag. The human-caption fields (`Caption-Abstract` / `XMP-dc:Description`) are never touched. Preview plus `[y/N]` confirm like tag-person (`--dry-run` shows old → new per file and writes nothing); requires a **fresh** photoindex (a stale cache could address the wrong file for a mutating write - hard block, not a warning); refuses in working-copy mode (§13d). The cache (`photos.user_comment` + `photo_fts.user_comment`) is patched in the same call for the paths that wrote, so `fha find --text` sees the new summary with no rescan. Exit 0, or 3 when any per-file read/write fails (partial success reported per file). <!-- PROPOSED AMENDMENT (photoindex-set-summary, pending owner approval): this set-summary paragraph, per AGENTS_TOOLING §8 spec-refinement gate. -->
...
<!-- PROPOSED AMENDMENT (04-fha-backup, pending owner approval; AGENTS_TOOLING §8 spec-refinement gate): this whole section is new with the `fha backup` build. -->
...
**`--include-assets`.** Zips each mapped root under its alias name (`photos/…`, `documents/…`), so an unzip restores a self-contained archive layout; the output notes the one wrinkle for external roots (move them back and keep `fha.yaml`, or leave them inside and delete the `roots:` mapping).

**Integrity.** After writing, every member's CRC is verified (`zipfile.testzip`) and the run reports `backup verified: N files, SIZE`. On any write or verify failure the partial zip is **deleted** and the run exits 3 - a backup that might be corrupt and says nothing is worse than no backup. Exit codes: 0 written+verified / dry-run / WC `--include-assets` refusal; 2 argparse only; 3 unresolvable root, refused destination, malformed `fha.yaml` (strict load), write/verify failure. There is deliberately no exit-1 arm: a partial or suspect backup is never a warning.
...
<!-- PROPOSED AMENDMENT (wave1-hygiene-ux, pending owner approval; AGENTS_TOOLING §8 spec-refinement gate): -->
...
| `fha backup [--to PATH] [--include-assets] [--dry-run]` (T C) | One dated zip of the whole archive, written **outside** it (default: the `{root}-backups/` sibling folder). Records-only by default - the photos/documents roots are excluded and named every run; `--include-assets` packs them under their alias names. Verified after writing; stamps `.cache/last_backup.json` so `fha doctor` reports the real last-backup date. Restore = unzip (§13e). <!-- PROPOSED AMENDMENT (04-fha-backup, pending owner approval): new row. --> |
