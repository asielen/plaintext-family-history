...
The deterministic write-backs for a person record's own frontmatter and prose. The group exists so that a field or a bounded prose edit every export decision or workbench button hangs on is a safe one-line command instead of a hand edit. Six verbs ship: `set-living`, `new`, `relate`, `estimate`, `edit`, `note`.
