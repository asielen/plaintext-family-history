...
`set` replaces exactly the fields given - coordinates (forgiving on shape, strict on ±90/±180 ranges, catching the classic swapped pair), the complete `alt_names:` list, the complete dated `history:` list (one `PERIOD | HIERARCHY` entry per flag/line) - each a FULL per-field replace; omitted fields untouched. `note` appends a DATED paragraph to the place's `notes:` (reference prose, SPEC §15 - loose citations welcome), preserving whatever is already written and re-emitting the key as a `notes: |` block scalar. `edit-note` rewrites ONE entry of that log, named by its exact current text - the same position-free per-entry edit `fha person edit-note`/`fha source edit-note` give the other running logs, with the same refusals (not found / ambiguous / empty replacement). Both are the same one-block surgical text edit geocode performs (comments and every other entry preserved). **No interactive `[y/N]` gate, deliberately:** geocode's gate exists because geocode INFERS coordinates; these verbs only ever write values the human personally supplied - typed into the command, or into the workbench form and applied through its dry-run-preview-then-confirm two-step - so the command itself is the confirmation the coordinate rule (AGENTS.md) requires. Exit codes: 0 written/dry-run · 1 unknown L-id or missing registry (with the `fha find` next step) · 3 invalid id shape, out-of-range coordinates, nothing-to-change, unreadable/unwritable registry.
