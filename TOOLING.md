...
- **`--related <P-id>`** (a person's world): **people** tied by relationship edges and co-occurrence (counts + provenance); **places** they recur in (claim `place`/`place_text` frequency); **shared entities** — orgs, clubs, employers, units from their claim values, ranked by how many others share each; their **sources** and **photos**. Backs the FAN view and the profile's Friends & Family suggestions.
- **`--related <L-id>`** (a place's world): every **claim** naming the place, the **people** in those claims (ranked by frequency — "who is most associated with Fairview"), the **sources** that reference it, **photos** geotagged within it (GPS ∈ the place, or `within:` children), and contained **micro-places**. The building block for "everyone connected to this place."
- **`--related <S-id>`** (a source's world): its **claims** and their statuses, every **person** named, **places** referenced, **other sources** that corroborate/contradict its claims (via `claim_links`), and sibling sources sharing its people or repository.
- **`--related <C-id>`** (a claim's neighborhood): its source, persons, place; claims it `corroborates`/`contradicts`; and **sibling claims** — same person + same type (the other residence claims, the competing birth dates), the cluster you'd weigh together.
- **`--related <H-id>`** (a hypothesis's neighborhood): the persons/places it concerns, the claims that bear on it, and the verifying claim if resolved.

**`fha find --related --date <EDTF>`** — the *time* neighborhood, orthogonal to ID type: all claims (and the people, sources, and photos behind them) whose dates overlap the given EDTF range, via the index's `date_min`/`date_max` bounds (§1) and photo EDTFs.
