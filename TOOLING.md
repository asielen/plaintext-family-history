...
fha claim new --source S-id --type TYPE --value "…" [--date DATE]
              ...
              [--status suggested|accepted] [--confidence high|medium|low]
              [--root PATH] [--dry-run]
```
...
The `fha person` sibling for a source record: deterministic write-backs to a source's own text rather than its Claims. Two verbs ship: `note` (appended free text was previously a hand-edit only) and `edit-note` (2026-07-17, the per-entry rewrite).
...
| `fha claim <C-id> […]` (T C) | The review gesture: move one claim's `status:` (`accepted`/`disputed`/`rejected`/`needs-review`/`superseded`), stamp `reviewed:` (§3b). Driven by `review-claims` and the report's prompts; `accepted` is the human gate. `--status` is now optional - at least one of `--status`/`--value`/`--date`/`--type`/`--place`/`--place-text`/`--persons`/`--confidence` is required, so a field-only correction needs no status move. `--dry-run`. |
