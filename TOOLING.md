...
The group's `edtf_resolved` is the **best-confidence variant's** EDTF: score by number of `!` components (D > M > Y), then `~` over `?`; deterministic tie-break by path. Only keyworded variants carry a date (above), so an EXIF-only back scan contributes nothing and can never manufacture a conflict against a reviewed front.
