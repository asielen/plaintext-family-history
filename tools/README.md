...
The deterministic source-record write-back group (TOOLING §3c sibling, plan 17). Two verbs
ship: `note`, an append-only addition to a source's `## Notes` section - the safe one-line
way to jot something down (from the phone, on the porch, mid-research-session) without
risking the `## Claims` fence or the frontmatter above it - and `edit-note` (2026-07-17),
which rewrites ONE existing Notes paragraph named by its exact current text (the
workbench's per-entry edit button). This module deliberately opens the `fha source`
namespace for future source-field verbs. The record is located by
