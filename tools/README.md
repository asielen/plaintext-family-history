...
| `--text "…"` | ✓ | notes_fts + re.search; photo captions searched when photoindex is fresh, else explicit skip-note; transcript FTS populated + queried (CLI + JSON/workbench), shipped M11.5 (D7) |
...
| W103 | ✓ implemented | Stale couple-folder bracket lists, and a couple-folder name missing its `+ second spouse` half (add-only via the shared `_lib.spouse_extended_base` rule: fires only when the folder's derived couple has exactly two members, the base has no `+` yet, and the base exactly matches one partner's name - an existing `+ …` or hand-crafted base is never rewritten); fires in `fha lint` and `fha views brackets`. Marks a child who joined other than by birth (`Ruth (adopted)`, `(step)`, …) from the backing claim's `subtype` (SPEC §12.2). ⚑ The cross-link entry for a person who occupies multiple Ahnentafel positions (`Thomas Hartley (also #128 - see 040)`) is **deferred** - the lowest-numbered folder is already the deterministic home (BFS-first), so this is a display-only follow-up. |
