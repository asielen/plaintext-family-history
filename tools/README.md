...
Two **companions** live beside this suite, neither load-bearing: [`browser-companion/`](../browser-companion/) (web capture - vendored ready-to-load into every archive at `.fha/browser-companion/` by `fha install`/`update-tools`, since it has no build step; developed here) and [`obsidian-templater/`](https://github.com/asielen/plaintext-family-history/tree/master/obsidian-templater) (installed into a user's own Obsidian setup, never vendored - hence the absolute link: an installed archive has no copy of it) (optional Obsidian Templater templates for new person/source notes; see [docs/USING_WITH_OBSIDIAN.md](https://github.com/asielen/plaintext-family-history/blob/master/docs/USING_WITH_OBSIDIAN.md) - absolute for the same reason `../docs/…` cannot be right in both the repo and an archive, where this file lands in `.fha/tools/`).
...
| Tool | File | Status |
|---|---|---|
| `fha reconcile` | `reconcile.py` | ✓ M11.1 - the general path healer (TOOLING §9): documents-side `files:` entries re-matched by filename S-id and rewritten line-surgically (unique hit heals; duplicate names ambiguous - reported, never guessed; vanished files and unlisted S-id files reported with next steps); photos side delegates to `photoindex.run_reconcile` when a catalog exists (report.py's orchestrator import precedent). A moved-AND-renamed file still heals when its S-id survives in the new name (§9's embedded-ID re-match, unlisted carriers only). Flags: `--root`, `--dry-run` (previews every re-tie), `--with-exif` (photos re-match). Exit 0 clean/healed, 1 anything left for a human (ambiguous/missing/unlisted, unreachable documents root), 3 on a corrupt/old-schema photo catalog. Working-copy is a clean no-op. Tests: `tests/test_reconcile.py` |
...
| `fha views brackets` | `views.py` | ✓ W103 bracket + `+ spouse` name refresh, W110 Ahnentafel placement, W119 direct-line stubs, W120 defaulted-slot note; `--fix` applies W103/W110, `--fix-promote` applies W119 under the same Apply? gate (refused together), `--realign` applies both halves in one previewed pass (destinations recomputed against the post-rename tree; promotions are skipped if any rename/move failed, so a couple is never split across two same-prefix folders), `--generations N` caps W119 depth, `--dry-run` previews, `--yes` skips the Apply? prompt for scripted/non-interactive runs (the preview still prints). Every fix path preflights the whole plan - folder-rename AND file-move destinations - and refuses it entire (exit 3, nothing written, same under `--dry-run`) when one is already taken. A brand-new couple folder never takes a placeholder name on either side: an unnamed even-slot partner yields the base to the named odd-slot one |
...
| `fha find --text "…"` | `find.py` | ✓ notes_fts + re.search; photo captions searched when photoindex is fresh (else skip-note); `transcripts_fts` populated by `fha index` from `role: extracted-text` companions and queried by both CLI text search and the JSON/workbench backend (D7, transcript search shipped M11.5) |
...
Views require a fresh `.cache/index.sqlite` (run `fha index` first). The per-person timeline/sources-index/draft-queue forms skip a stub person with a plain note and exit 1 - companion views are curated-person files (SPEC §16); covered by `tests/test_views_stub_guard.py`. A `--all-curated` / `refresh` batch survives one person's failure (a hand-written file in the way, a folder the index no longer finds): it warns, writes the rest, and exits 1 with `data['skipped']` so a partly written batch never reads as clean. `fha find` uses the index when present, warns when it is stale, and falls back to a tree scan only when the index is absent or unreadable; `fha doctor` degrades gracefully without caches. Both `.cache/index.sqlite` and `.cache/photos.sqlite` carry a `meta.schema_version` row plus `PRAGMA user_version`; missing, old, corrupt, or unreadable caches are treated as disposable and rebuilt by `fha index` / `fha photoindex`.
Generated files carry the `<!-- GENERATED … -->` header and must not be hand-edited.
`--format html` renders any content view as a standalone single-file page under `generated/views/` (same queries and lines as the `.md` twin; marker on line 1 before the doctype; inline `design/view.css`; visible not-for-publication banner; `[[ID]]` tokens as plain styled text; exits 0 because `generated/` is never indexed). `views refresh --format md|html|both` selects the set(s); `views clean` sweeps `generated/views/` by the same marker-per-file rule (a hand-written file there survives). Both sweeps exit 0 when they finish clean: a deleted people/-tree companion has its `notes_fts` and `person_files` rows dropped in the same pass, and a companion write updates the same rows - so a regenerated view is searchable immediately and a deleted one stops being (`_lib.sync_generated_view_rows`). The `fha index` reminder prints when there is no index to update, which is not a fault and stays exit 0; a row update that *failed* is different - the files no longer match the rows and #37 keeps the watermark silent about it, so every write path (the three content views, single-person and batch alike, plus `refresh`) and `clean` exit 1 with `data['index_stale']`. Exit 1 also for a file that would not read or delete during a clean (named, skipped, counted in `data['unreadable']` / `data['failures']`). Conventions: TOOLING §7 D11; covered by `tests/test_views_html.py` and `tests/test_views_index_sync.py`.

## Implemented tools (milestone 3)

| Tool | File | Status |
|---|---|---|
| `fha photoindex [--full]` | `photoindex.py` | ✓ M3.1 - schema, exiftool scan (incremental by mtime/size; `--full` rescans all; `photos_ignore:` patterns in fha.yaml prune subtrees/files, and the pattern set plus the configured `roots: photos` value are stamped into the catalog's `meta` row at the end of each scan so a later edit to either reads `stale` (with the changed setting named) instead of `fresh`; paths travel to exiftool via stdin (`-@ -`) so no OS command-line limit applies; unreadable files are reported and skipped, never fatal - a new/changed one that could not be read pulls the cache mtime back so the catalog keeps reading `stale` until it can be, and when that pull-back itself fails the run says so and exits 1 rather than leaving a catalog that only looks current), variation grouping (a `reconcile`-flagged `MISSING:` row groups by the alias under its prefix and contributes no date; a live variant leads the group), `DATE:` precision-keyword + EXIF-value date resolution (SPEC §20; no keyword = undated by design; **the letter grammar `Y!M!D!`/`Y!M!`/`Y!M~`/`Y!`/`Y~` is the only form read** - a digit-bearing keyword like `DATE: 1880`, `DATE: 1942!-11!-25!` or `DATE: [..1900]` is outside the spec and leaves the photo undated, counted into the summary's `nonspec_date_keywords` and explained in a plain-language scan note; `?` and an omitted per-component marker both mean unknown, so `Y!M` resolves to the year alone and `YMD` to no date), person resolution |
| `fha photoindex find` | `photoindex.py` | ✓ M3.2 - `--person`/`--keyword`/`--edtf`/`--text`/`--under`/`--not-under` filters (AND'd at the group level when combined); one path per group by default, `--files` for raw rows; `--under`/`--not-under` match a `MISSING:` row on the folder it vanished from |
...
| `fha photoindex reconcile [--with-exif]` | `photoindex.py` | ✓ M3.4 - re-matches a moved file by its embedded `SOURCE:` keyword (`--with-exif` only); unmatchable rows are flagged `MISSING:` in the cache; new on-disk files are counted, not scraped; `photo_fts` is re-keyed alongside every other path-keyed table; a failed staleness pull-back is reported and exits 1 (`stale_hold_failed`) |
| `fha photoindex tag-person <P-id> [--from-face-tag TAG \| --paths PATH...] [--dry-run]` | `photoindex.py` | ✓ M3.4 - a photo flagged `MISSING:` is never offered (face-tag selector skips it; naming one in `--paths` is refused toward `reconcile`); preview -> interactive `[y/N]` confirm (or `--dry-run`) -> one `exiftool -keywords+=` write per candidate -> `photo_people`/`photo_keywords`/`photo_fts` cache update for whichever candidates' writes succeeded |
| `fha photoindex gallery [--person P-id \| --keyword TERM \| --edtf EDTF \| --text TEXT] [--out FILE]` | `photoindex.py` | ✓ M3.6 - same filters as `find` (≥1 required, AND at the group level, same P-id/EDTF validation) -> one disposable self-contained HTML page under `generated/gallery/` (`--out` overrides; the landing name carries every filter, and a filter value `_slugify` would flatten into another's keeps a short `-h……` digest so two distinct filters never share a page); decade sections (a slash/bracket-qualified date keeps its range/qualifier in the label, e.g. "1912 to 1915", "before 1900") + "Undated" tail + a "Verify these" tail for weak `--person` matches (its printed `tag-person` command shell-quotes the example path); renderable `.jpg/.jpeg/.png/.webp/.gif` get lazy `<img>`, other formats a placeholder tile; a `reconcile`-flagged `MISSING:` row (or a group whose every variant is one) is dropped rather than rendered as a broken link; GENERATED marker on line 1 (overwrite-by-default, refuses a marker-less target); refuses in working-copy mode (exit 0, `status=working-copy`). Exit 3 on absent/unreadable/old-schema cache or bad input; stale warns and still builds (exit 0); zero matches writes nothing (exit 0) |
...
| §1b Worth a second look | ✓ | Parked needs-review claims (oldest `reviewed:` first) and accepted `confidence: low` claims - counts plus the oldest 3 each, per the no-flood rule; leads for research-next, never defects (owner decision 2026-07-22) |
...
| §5b Answerable questions | ✓ | Open questions (from `notes/questions.md` and `## Q:` blocks in person research files under `people/` - the same scope as lint's E009 scan; keyed by file so a heading recurring across files can't shadow) whose referenced `[C-id]` is now `accepted`, or whose referenced `[P-id]` now has all its required-vitals accepted claims; proposals only - printed, never executed |
| §6 Photo triage | ✓ | Embeds `photoindex.run_triage(top=10)`; absent/unreadable photo index reported, not treated as an error. The scan's own note prints first (it is often WHY the catalog is absent or out of date): files exiftool could not read are named, and when any of them are new or changed - the ones `run_scan` holds the catalog's date back for (`unreadable_unindexed`) - the section says the catalog stays marked out of date until they can be read and names `fha photoindex` as the fix, so the resulting "photo index is stale" warnings elsewhere are explained rather than looking like a second, permanent fault. A triage candidate whose group is named by a `MISSING:` row is listed under the path it had with the `fha photoindex reconcile --with-exif` next step instead of an `fha process` suggestion no one could type |
...
| §7b Promotion candidates | ✓ | Direct-line stubs (the W119 set via the shared Ahnentafel walk, closest generations first, each offering `fha person promote`) + stubs with accepted claims ≥ `promotion: claims_threshold:` (fha.yaml, default 5; non-direct claim-heavy stubs noted but not offered the verb - the connections design fork). Stateless; leads, never defects |
...
|---|---|---|---|
| `fha claim <C-id> [<C-id> …]` | `--status accepted\|disputed\|rejected\|needs-review\|superseded`, `--value "…"`, `--date EDTF`, `--type TYPE`, `--place L-id`, `--place-text TEXT` (independent keys - SPEC §15; may be given together), `--persons P-id[,P-id…]`, `--confidence high\|medium\|low`, `--reviewed DATE`, `--root`, `--dry-run` | ✓ | `--status` is now OPTIONAL - at least one of `--status`/`--value`/`--date`/`--type`/`--place`/`--place-text`/`--persons`/`--confidence` is required, so a field-only correction (fixing a typo'd `--value`, attaching `--persons`) needs no status move. **Several C-ids at once** apply one status move to each (TOOLING §3b, 2026-07): status-only - any field-edit flag with more than one id is refused (a field correction is inherently per-claim); all-or-nothing - every id is validated (shape and existence) before any write, so one bad id refuses the whole batch naming the id and the fix; duplicate ids are deduped preserving order with a note; one `--dry-run` previews every edit, and the live run loops the same surgical single-claim write, folding the per-claim `fha index` reminders into one. The five `--status` choices are the SPEC §8.1 review outcomes; `--status accepted` still always stamps `reviewed:` (the given `--reviewed`, else today) - only the human moves a claim to `accepted` (lint E006), and directing this tool *is* the human's decision. `disputed`/`rejected`/`superseded` change status rather than delete (trail preserved). `--type` corrects the claim type (SPEC §8.2 vocabulary); changing TO `relationship` is refused (it needs a `roles:` map this command does not build - see `fha person relate`/`fha confirm cooccur`). `--place`/`--place-text` each set ONLY their own key (SPEC §15: the normalized link and the source-as-written wording legally coexist; backfilling `place:` never alters `place_text:`). `--persons` REPLACES the whole `persons:` list (every id must already have a person record). `--confidence` sets the evidence-quality level (SPEC §8.5), independent of `--status`. A YAML block-scalar `value:` is refused, not corrupted. The claim is located by its OWN `id:` key line with a parse-back cross-check, so an `id: C-…` quoted inside another claim's block scalar (`notes: \|`) never draws the edit onto the quoting claim. A pre-existing duplicate C-id refuses with the E001 repair path (`fha id mint C`), not the corruption warning. Missing C-id → exit 1; re-run `fha index` after a write |
| `fha claim new` | `--source S-id` (req), `--type TYPE` (req), `--value "…"` (req), `--date DATE`, `--place L-id`, `--place-text TEXT` (independent; both may be given), `--persons P-id[,P-id…]`, `--subtype WORD`, `--status suggested\|accepted` (default `accepted`), `--negated` (SPEC §8.6 confirmed absence: writes `negated: true` + `evidence: negative`), `--confidence high\|medium\|low`, `--root`, `--dry-run` | ✓ plan 17 | Mints a brand-new claim onto an existing source's `## Claims` block. Not reachable through the ordinary `claim` subparser (its positional C-id shape would misparse `new` as a claim id) - `fha.py` intercepts `fha claim new …` before the flat parser ever sees it, the same early-interception pattern `fha gedcom import` uses. `--type relationship` is refused - it needs a `roles:` map this command does not build; use `fha person relate` (hypothesis) or `fha confirm cooccur` (sourced) instead. `--persons` is optional (a claim may legitimately be minted before its persons are linked; the result warns naming the follow-up). `--confidence`, when omitted, defaults from the source's `source_type` per SPEC §8.5 (`vital-record` → high, `interview` → low, everything else → medium), with a message naming what was chosen. Defaults to `--status accepted` - typing this command IS the review (`reviewed:` stamped to today automatically); an AI caller must pass `--status suggested` explicitly. `--date` accepts EDTF or clear plain words ("about 1880"). Every write funnels through the same claim-append helper `fha confirm cooccur` uses, so a rewrite that would corrupt the block is refused, never saved |
...
combination, `--persons` list replacement, `--place`/`--place-text` key independence, and the
...
The deterministic person-field write-back group (TOOLING §3c). Nine verbs ship:
...
`note` (append-only Stories/Research Notes); `edit-note` (rewrite one existing
entry, located by its exact current text); and `promote` (graduate a direct-line stub
into its derived couple folder - the TOOLING §5 carve-out, M11.4). Every verb locates its record by scanning `people/` directly
(`_lib.find_person_record_path`, shared with `fha confirm draft`), so a stale or absent
index never blocks a write - with one carve-out: `promote` reads the index to derive
WHERE the record files (never WHETHER; a stale index there is a plain refusal naming
`fha index`, since a wrong couple-folder destination is worse than a wait). Nothing flips `living:` automatically - accepting a `death`
...
| `fha person set-sex <P-id> M\|F\|intersex\|unknown` | `--root`, `--dry-run` | ✓ 2026-08-15 | Sets or corrects `sex:` (SPEC §9 vocabulary, case-folded on input: `f` → `F`), the one frontmatter fact the Ahnentafel derivation reads. Same surgical single-line edit as `set-living` (pre-write guard, merged-tombstone refusal naming the survivor, `already` no-op, `--dry-run` diff); a missing key is inserted in template order (after `name`/`name_variants`/`face_tags`, before `living`). A live change ends with the reindex-then-brackets nudge - "run `fha index`, then `fha views brackets` to check the folder numbering, `--realign` to apply any shift" - the reminder a hand edit could never print, in the order that works: the placement is derived from the INDEXED `sex:`, so before the rebuild `brackets` still reports the old numbering and `--realign` refuses on the stale index. Exit 1 = P-id not found; exit 3 = invalid value/id, merged tombstone, guard refusal |
| `fha person set-profile-photo <P-id> PHOTO` | `--root`, `--dry-run` | ✓ 2026-07-17 | Sets the record's `profile_photo:` field (SPEC §9 - the person-page portrait and tree thumbnail); the workbench set-profile-photo picker echoes exactly this command. `PHOTO` is a filename, path, or S-id, written as given and resolved leniently at site-generation time (an unresolvable value just means no portrait shows - never a write-time refusal, the same rule as a missing hero/embed reference). Same surgical single-line frontmatter edit as `set-living`: pre-write guard (frontmatter re-parsed; only `profile_photo` may change), merged-tombstone refusal naming the survivor, `already` idempotent no-op, `--dry-run` unified diff. The value routes through the shared `yaml_inline` quoting rule, so a ` #` or `: ` in a common filename (`Grandpa #2.jpg`) can never truncate as a YAML comment. Exit 1 = P-id not found (+ `fha find` next step); exit 3 = invalid id shape, merged tombstone, guard refusal, unreadable/unwritable file |
| `fha person new "Name"` | `--sex M\|F\|intersex\|unknown`, `--gender TEXT`, `--birth DATE`, `--death DATE`, `--birth-place TEXT`, `--death-place TEXT`, `--root`, `--dry-run` | ✓ plan 17 | Mints one P-id and writes its stub under `people/stubs/` - the parity command behind every "+ add person" workbench button, and the deliberate counterpart to `fha stubs` (that tool mints stubs FOR references already unresolved in claims; this one mints a stub a human deliberately starts from nothing). `--sex` has no argparse `choices=` so an unrecognised value gets the plain, gender-glossed refusal message instead of argparse's bare "invalid choice" text (case-folded, so `m`/`M` both work). `--gender` is free text, omitted unless there is something to record. `--birth`/`--death` are PROVISIONAL, unsourced estimates (SPEC §9) - strict EDTF as-is, loose wording normalized with a plain gloss ("about 1870" → "recorded as 1870~"), anything unreadable refused with the two-example date error before anything is minted. Every input is validated BEFORE `mint_ids` draws an id, so a bad flag never burns a fresh P-id. `--dry-run` still draws a real id (so the preview's filename/content match a live run exactly) but never writes it to any file. Never overwrites an existing target (next-to-impossible after `mint_ids`'s own collision scan, but still a plain refusal, not a silent clobber) |
| `fha person promote <P-id>` | `--into FOLDER`, `--root`, `--dry-run` | ✓ | Direct-line stub graduation (TOOLING §3c, the §5 carve-out): surgical `tier:` flip + move out of people/stubs/ into the derived couple folder (created if absent) + SPEC §16 research scaffold from `people/_TEMPLATE.research.md`, transactional with rollback, shared engine with `fha views brackets --fix-promote` (`_lib.promote_person_record`). Strict-fresh index required (the one person verb that reads the index - to decide WHERE, never WHETHER). `--into` overrides the destination for a direct-line person only (one folder under `people/`, never stubs/ or connections/). Non-direct people refused plainly (v1 scope - curating beyond the direct line is an open design decision); already-curated already-filed = clean `already` no-op; a curated record still in stubs/ is finished, not refused. Exit 0 ok/already/dry-run · 1 not found · 3 refusals |
...
| `fha person estimate <P-id>` | `--birth DATE\|-`, `--death DATE\|-`, `--birth-place TEXT\|-`, `--death-place TEXT\|-`, `--root`, `--dry-run` | ✓ plan 17 | Writes the provisional, unsourced `birth:`/`death:` estimate SPEC §9 documents as a normal starting state - superseded by an accepted vital claim once one exists. At least one of the four flags is required. `-` by itself clears a field (removes the line). Strict EDTF or loose human wording (`normalize_date`) accepted for either date; both dates are validated BEFORE any file is touched, so a bad second date leaves the first one unwritten too. `--birth-place`/`--death-place` are the same provisional standing for the WHERE (free text, unvalidated - "born in Kansas, no idea when" is real family knowledge): a place may be recorded with or without its date, and the workbench summary row shows either alone. A field already recording the requested value (or already absent, for a clear) is a silent per-field no-op - the same idempotence rule as `set-living` |
...
| `fha person edit-note <P-id> --section stories\|research --old-text TEXT --text TEXT` | `--root`, `--dry-run` | ✓ 2026-07-17 | Rewrites ONE existing append-log entry (a blank-line-separated paragraph run - exactly what one `note` append wrote), leaving every other entry untouched; the workbench's per-entry edit button. The entry is named by its exact current text, not by position (display filters strip drafts/private markers, so a rendered index can't be trusted; text can): no match = a plain "reload the page" refusal, more than one match = a plain "edit the file" refusal, nothing written either way. An empty `--text` is refused - removals stay a deliberate hand edit. Warns (exit 1) on unbalanced `[[`/`]]` in the new text and on an edit that changes the entry's `<!-- private -->` marker counts |
...
The deterministic source-record write-back group (TOOLING §3c sibling, plan 17). Three
verbs ship: `note`, an append-only addition to a source's `## Notes` section - the safe
one-line way to jot something down (from the phone, on the porch, mid-research-session)
without risking the `## Claims` fence or the frontmatter above it; `edit-note`
(2026-07-17), which rewrites ONE existing Notes paragraph named by its exact current text
(the workbench's per-entry edit button); and `extract` (M11.5), which dumps a source
PDF's embedded text layer into a derived `[Page N]`-labeled companion. This module
deliberately opens the `fha source` namespace for future source-field verbs. The record
is located by scanning `sources/` directly (the shared `_lib.find_source_record_path`,
sibling of `find_person_record_path`), so a stale or absent index never blocks the write.
...
| `fha source edit-note <S-id> --old-text TEXT --text TEXT` | `--root`, `--dry-run` | ✓ 2026-07-17 | Rewrites ONE existing `## Notes` paragraph, named by its exact current text (`_lib.replace_paragraph_in_section` - the same matcher as `fha person edit-note`); every other note stays as written. No match / an ambiguous duplicate / an empty replacement are plain refusals (removals stay a hand edit). The same Claims-regression guard, duplicate-heading refusal, and exit-code mapping as `note` above |
| `fha source extract <S-id>` | `--pages RANGES`, `--root`, `--dry-run` | ✓ M11.5 | Dumps the source PDF's embedded text layer into a derived companion (`{slug}-extracted-text_{S-id}.md`, filed beside the PDF, `role: extracted-text, derived: true` appended to the record inventory through the shared `_lib.append_file_entry_to_record`). `[Page N]` labels use the PDF's own pagination so `anchor: "page N"` claims line up in any copy; a page with no text layer gets an honest placeholder line; an all-image PDF is a warning refusal toward vision/OCR with nothing written. Reads the primary PDF (or the single non-derived PDF); several PDFs without a primary refuse rather than guess; re-running is a clean `already` no-op (extraction never overwrites). pypdf is optional - absent, the verb refuses naming `python -m pip install pypdf`. Exit 0 ok/already/dry-run · 1 not-found/working-copy/no-text · 3 refusals. Tests: `tests/test_source_extract.py` (hand-rolled minimal PDFs) |
...
| Feature | Status | Notes |
|---|---|---|
| Schema (`.cache/photos.sqlite`) | ✓ | `meta` (schema version + the build configuration the catalog was scanned with), `photos`, `photo_groups`, `photo_keywords`, `photo_face_regions`, `photo_people`, `photo_fts`; face regions cache XMP names/types/area JSON so weak person resolution can be rebuilt without re-scraping unchanged images |
...
| `fha photoindex find` | ✓ (BUILD.md M3.2) | `--person P-id` (must be a P-id - wrong-type or malformed ids are rejected), `--keyword TERM` (case-insensitive substring), `--edtf EDTF` (must be valid EDTF; bounds-overlap against each photo's own `edtf`), `--text "…"` (`photo_fts`), `--under PATH` / `--not-under PATH` (subtree of the photos root, relative or `photos/…` alias form; `--under` alone is a valid filter); filters AND together **at the group level** (a filter matching any variant matches the whole logical photo; a group with any variant under `--not-under` is dropped). Default dedupes matches to one row per group (`primary_path`); `--files` returns every raw row of each matched group (including sibling variants that didn't themselves match). Absent/unreadable/incompatible-schema `.cache/photos.sqlite` → clear error, exit 3; stale → warns but still queries |
| `fha photoindex triage [--top N] [--under PATH] [--not-under PATH]` | ✓ (BUILD.md M3.3) | Candidates = `photo_groups` rows where no member photo has `source_id` set (and, with `--under`/`--not-under`, with/without a member under that subtree). Score (TOOLING §15b): +3 any member has a caption, +2 any member's `photo_people` row is `via='pid-keyword'`, +1 any member's `edtf` has no `~`/`?` marker (`Y!` confidence or better), +1 any member is a back scan (`variant_role` is `back` or `back-crop` - the TOOLING §6 compound for a `-back` variant and a crop of one; a freeform role that merely begins with those letters is not one), −2 no caption anywhere in the group **and** some member's `user_comment` starts with `AI:`/`Model:`. Sorted by `(-score, primary_path)`; `--top` (default 10) caps the list. Same absent/unreadable/stale handling as `find` |
...
| `fha photoindex gallery [--person P-id \| --keyword TERM \| --edtf EDTF \| --text TEXT] [--out FILE]` | ✓ (BUILD.md M3.6) | Reuses `find`'s exact filter-to-groups logic (`_matched_filter_groups`, ≥1 filter required, same P-id/EDTF validation) to build one disposable self-contained HTML page under `generated/gallery/` (`{slug}_{P-id}.html` for a bare person filter, with the filter tokens appended when `--person` carries extra filters so distinct filter sets never collide, `gallery_{filter tokens}.html` otherwise; `--out` overrides; distinct EDTF forms keep distinct tokens - `1912` vs `1912~` -> `edtf-1912` vs `edtf-1912-approx`, and an interval's halves are mapped one by one so `[..1900]/1910` no longer lands on the plain `1900/1910`'s page; the free-text filters (`--keyword`, `--text`, `--under`, `--not-under`) take a short stable `-h……` digest whenever the slug is lossy, so `--under A/B` and `--under A-B` keep separate pages while the readable stem survives; `--under`/`--not-under` resolve to the catalog's alias form first, so the several ways of naming one folder share one page; `file://` hrefs resolved through `fha.yaml` roots; the printed path is authoritative rather than predicted). One row per logical photo (group `primary_path`) with variants as zero-JS `<details>` chips; decade sections newest-first (decade read from the EDTF literal, not a widened bounds midpoint) + an "Undated" tail (open-ended/undecodable dates land here) + a count strip (headline counts only confirmed matches; weak `--person` matches are named in a separate clause). A row's date label keeps a slash-interval or bracket-qualified date's range/qualifier ("1912 to 1915", "before 1900", "after 1900") rather than collapsing to its first or boundary year. A `reconcile`-flagged `MISSING:` row never becomes a tile or chip - a group with some variants still on disk renders from those, one with every variant `MISSING:` is dropped entirely, since (unlike `find`) a gallery row is meant to be clicked. With `--person`, weak `face-tag`/`name-match` groups fall into a "Verify these" tail pointing at `tag-person`, its printed command shell-quoting the example path (a space or shell punctuation in the path would otherwise split it into two arguments). Renderable `.jpg/.jpeg/.png/.webp/.gif` get a lazy `<img>`; other formats a placeholder tile that still opens the file. Inherits the single-file-HTML conventions (TOOLING §7 D11): GENERATED marker on line 1 before the doctype (overwrite-by-default; a marker-less target is refused, exit 3), inlined `design/view.css`, private-artifact banner, zero network requests, no JavaScript. No derivative pipeline in v1 (`--thumbs` scaffolded as a documented v2). Refuses in working-copy mode (broken links; exit 0, `status=working-copy`). Absent/unreadable/old-schema cache or bad input → exit 3; stale warns and still builds (exit 0); zero matches writes nothing (exit 0); a build prints the path and its `file://` URL |

Test fixture: `tests/fixtures/photo-fixture/` - 4 placeholder JPEGs with real embedded metadata (written via exiftool, not a code-level stub): a front/back variation pair with disagreeing `DATE:` keywords (written to exercise `date_conflict`), a photo carrying a `SOURCE:` keyword (exercises source-id grouping), and one ungrouped photo. ⚑ The embedded `DATE:` keywords are still in the retired digit form (`DATE:1880!`, `DATE:1881!`, `DATE:1902-06!`), so as of the 2026-08-16 letter-form-only rule they no longer resolve to a date; every test in `test_photoindex.py` injects its own keywords over a copy of the fixture, so nothing depends on them, but the files want re-keywording (`DATE:Y!` plus a `DateTimeOriginal`) next time a machine with exiftool touches them.

Automated tests: `tests/test_photoindex.py` (stdlib `unittest`, no new dependency) monkeypatches `photoindex._run_exiftool` (and, for tag-person and set-summary, `_run_exiftool_write`/`_run_exiftool_read_comments`/`_run_exiftool_write_comment`/`builtins.input`) to inject canned JSON rows or simulated confirm answers over a copy of the fixture, covering grouping/date-conflict/pid-keyword resolution, the letter-form-only date rule (SPEC §20's keyword table walked row by row; a bare year, the digit-plus-marker form and a raw EDTF string each leave the photo undated, are counted into `nonspec_date_keywords`, and draw the scan's explanatory note only when the count is non-zero), face-region caching, stale-index-disables-weak-resolution behavior, fresh-index weak-resolution refresh from cached regions, `--full` vs. incremental scan equivalence, reconcile's rematch/missing/untracked outcomes (including `photo_fts` re-keying), tag-person's plan/confirm/dry-run/write paths (including `photo_fts` refresh and per-candidate partial-failure handling), and set-summary's compose rule (AI replace / append / human-text preservation), group-vs-path addressing, cache+FTS mirror, stale-index block, dry-run/decline safety, and per-file partial failure. `GalleryTests` covers the gallery: one row per group with variant chips, group-level filter AND with gallery/find agreement, decade sections + undated tail, the confidence split, renderable-vs-placeholder tiles, `file://` resolution through external roots, marker overwrite / foreign-file refusal, default landing + `--out` + site non-collision, zero-match no-write, filter/P-id/EDTF validation, stale-warns-vs-absent-fails, self-contained HTML, and the banner/marker (the working-copy refusal lives in `tests/test_working_copy.py`). Run with `python -m unittest tests.test_photoindex -v` from the repo root.
...
| `fha process refile <S-ID> [--file NAME] --to photos\|documents [--dest SUBPATH] [--yes] [--dry-run]` | `process.py` | ✓ usage-feedback item 3 (2026-07-23) - the sanctioned cross-root correction: record found by S-id tree scan; single non-derived entry or `--file` (derived only when named); already-under-target-root refuses toward `fha reconcile`; `--dest` required into photos (containment-validated), defaults to `documents/{type}/` into documents; last-chance rename into photos (`original_filename` restored, else `_S-id`/role/copy stripped), §13-grammar rename into documents; `SOURCE:` keyword embedded into photos (exiftool absent / unsupported format = WARN + proceed); photo→doc confirms ([y/N] or `--yes`, non-interactive without it refuses); value-exact inventory rewrite + dated `## Notes` provenance in one atomic write; transactional reverse-order undo; exits 0/1/3; intercepted in `fha.py` before argparse (the `fha claim new` pattern) |
...
| `/api/run` parity table | ✓ | A whitelist of 25 verbs (`claim.review`, `claim.new`, `confirm.xref`, `confirm.cooccur`, `confirm.dismiss`, `person.set-living`, `person.set-profile-photo`, `person.set-sex`, `person.new`, `person.relate`, `person.add_family`, `person.estimate`, `person.edit`, `person.note`, `person.edit_note`, `source.note`, `source.edit_note`, `place.set`, `place.note`, `place.edit_note`, `process.file`, `capture.path`, `site.publish`, `index.rebuild`, `home.edit`), each mapped to (schema, engine call, CLI-echo formatter, reindex policy). An unknown verb or an argument outside the verb's schema is a plain 400, never a silent drop. Every verb drives the SAME `run_*` engine the CLI uses, and every apply response's `cli_echo` field is the literal `fha …` command that would do the same thing (the parity rule made visible) |
| Reciprocal family ties | ✓ | `person.relate` defaults `reciprocal` to TRUE (owner decision, review 2026-07-16) and `person.add_family` always relates reciprocally: an unsourced tie added from either person's page lands on BOTH records, so it shows from both. An explicit `{"reciprocal": false}` still records a one-sided belief; the CLI echo names the `--reciprocal` that actually ran. `person.add_family`'s typed-name path is all-or-nothing: it mints the stub then relates, and a relate refusal (stale/merged subject, bad relation) unlinks the just-minted stub and says so - a failed combined action never leaves an orphan record behind |
| Per-entry log edits | ✓ | `person.edit_note` / `source.edit_note` / `place.edit_note` rewrite ONE entry of a Stories/Research Notes/source-`## Notes`/place-`notes:` append-log, named by its exact current text (`replace_paragraph_in_section`, or the same match over the registry value for places); a stale page (entry edited since load) or an ambiguous duplicate refuses with a plain message instead of mis-editing. Every rendered running log now carries per-entry edit buttons |
| Place registry verbs | ✓ | `place.set` (coordinates / also-known-as / names-over-time, each a full per-field replace), `place.note` (dated research note appended to `notes:`), and `place.edit_note` (rewrite one existing note by its exact text) drive `fha places set` / `note` / `edit-note`. The dry-run diff + the human's own Apply are the confirmation the AGENTS.md coordinate rule requires - the values are the human's own, never inferred |
...
| `/api/pickfile` | ✓ | Opens the OS's own file-picker window (tkinter, in a one-shot SUBPROCESS - Tk is not thread-safe and every request runs on its own thread) and returns the chosen path; a browser page cannot read a local file's full path, so serve does it, since it runs on the same machine. Nothing is read/moved/registered by the pick - the path just fills the register-by-path form, and the write still goes through `capture.path`'s preview/apply. One dialog at a time (its own lock, never the mutation lock); no-tkinter machines and a cancelled dialog get plain messages and the text field keeps working |
...
| Person page (M8.2) | ✓ | Summary block from accepted vital claims (low-confidence vitals flagged); biography + Stories HTML (stdlib markdown→HTML + token swap, read from the person `.md`); timeline (decade-grouped, same shape as `fha views timeline`; standalone = accepted only with a "low confidence" flag on thin facts, linked/workbench also shows needs-review marked "unconfirmed - parked {date}", suggested excluded everywhere - owner decision 2026-07-22); sources index grouped by `source_type` (same two-table UNION as `fha views sources-index`); photo strip (`photo_people`, one entry per variation group; a `reconcile`-flagged `MISSING:` row is dropped at the query - it has no file to render and would otherwise win the one-per-group pick and take its still-present siblings off the page); Friends & Family from the `relationships` edges |
...
| `--standalone` (default) | ✓ | Self-contained, redacted snapshot. Living/unknown persons - and `restricted` persons (any value, read from the person file) - get **no page** and render as "Living Person"; restricted, DNA, and `rights.publication_ok: false` sources get **no page** and render as "Restricted…" (a free-text `restricted: by-request` source type is read from the file beyond the index's 0/1); a single `restricted` claim is withheld from the summary, timeline, source page, and place page even when its source publishes; a restricted name variant (a deadname) resolves internally - `[[prior name]]` still links - but renders the person's unrestricted display name (SPEC §18); source pages publish only **accepted** claims (`needs-review` is parked research state, withheld along with `suggested` AI drafts and `rejected`/`superseded` - matching the timeline; owner decision 2026-07-22); image assets become resized (≤1200px), EXIF-stripped derivatives copied into `site/media/` under collision-free names (stem + a hash of the alias path, so two same-stem photos never overwrite each other). A page is linked only if it was generated (no dangling redacted links) |
| `--linked` | ✓ | Local developer preview: real archive paths (no copies), no redaction. Mutually exclusive with `--standalone` |
| Photo references (`profile_photo:`, hero, prose embeds) | ✓ | Resolved through the catalog first (a photo still on disk preferred over a `reconcile`-flagged `MISSING:` row, which cannot be opened), then by path, then - for a bare filename only - by scanning the photos root. That scan honors `photos_ignore:` (fha.yaml): a subtree excluded from the library is never guessed from, matching the scan's own pruning. An explicitly written path is still honored, ignored folder or not. The living-tagged-photo gate matches a tag on either spelling of the path, so a photo whose file has come back before the next rescan cannot publish past it |
...
| Place page (M8.3) | ✓ | Name, coordinates as an **embedded OpenStreetMap iframe** with a marker plus the plain map link (2026-07-17, owner decision - the iframe degrades to an empty ruled frame offline, the link always works), the registry's `notes:` reference prose, dated `history:`, claims naming the place, contained micro-places (`within:` children, linked), and a people-frequency list. Workbench mode adds the edit affordances (coordinates / also-known-as / names-over-time / add-a-note) driving `fha places set`/`note`. `[L-id]` tokens in prose now link here. People links redact as everywhere; the people-frequency list omits redacted persons entirely so a standalone place page never names a living person |
...
status policy (standalone accepted-only with low-confidence flags; linked
needs-review-in marked unconfirmed; suggested-out) + decade grouping,
grouped F&F and sources),
...
| `photos/` | ✓ | Union of `photo_people` (`pid-keyword`/`face-tag`/`name-match`, already computed by `fha photoindex`) expanded to each match's full `photo_groups` variation group, plus image-suffixed asset files from included sources. Missing/unreadable/stale photo index refuses (exit 3) unless `--no-photos`; an individual photo file missing on disk (stale cache entry) is listed in stderr and README.txt, same as a missing source asset, never silently dropped. A row `fha photoindex reconcile` has flagged `MISSING:` is never copied (a packet is a physical bundle) but is named in the README under the path it had - no `MISSING:` prefix leaks into the export - with the stderr warning naming `fha photoindex reconcile --with-exif`; its group siblings still ship, and both README cautions follow the logical photo rather than the one variant: as long as any variant of that group is copied, the vanished side's person tags still count toward the living-person caution and the group still counts toward the "matched by name only" count. A group that put no file in the bundle contributes neither caution - nothing of it is there to caution about |
...
Automated tests: `tests/test_packet.py` builds a synthetic `.cache/index.sqlite` (and, where needed, a synthetic `.cache/photos.sqlite`) directly from `index.py`'s/`photoindex.py`'s DDL, covering the curated/living gates, strict stale-index refusal, restricted/DNA source filtering (both directions), the other-living-person caution (`living: true` and `living: unknown`), timeline source filtering, missing asset/photo reporting, `--include-research` with no research file, output conflict/overwrite/dry-run (including `--dry-run --overwrite` together) behavior, external `--out` display, the missing/absent/stale photoindex paths, photo-group expansion (a person tagged on one variant pulls in its siblings), the `MISSING:` rows reconcile leaves behind (skipped from `photos/` and named in the README by their real path; the group-level cautions - a name match whose only matched variant vanished still counts while a live sibling ships, a wholly-missing group counts for neither caution, and a group verified anywhere by P-id keyword or face tag is never called a name match), a per-file copy failure (mocked `shutil.copy2`), and a structural build failure (mocked `_zip_directory`). The generalized `restricted` contract (by-request never opens, dna needs `--include-dna`, restricted subject refused, restricted claim dropped from the timeline) is covered by `tests/test_privacy_restricted.py`. AI-draft withholding in the profile copy (draft absent, accepted prose shipped with markers removed, damaged marker leading to `write-failed`, the research-copy caution) is covered by `tests/test_packet.py`; the id-less and unparseable restricted-claim fail-closed paths, the timeline consistency pin, and the wrapped/nested alias-mirror forms by `tests/test_privacy_restricted.py`.
...
| `fha places geocode` | ✓ (M6.3) | Backfills `coords` (and proposes `alt_names`) for registry places missing coordinates (`--all`) or one place (`--place L-id`). Gazetteer is the offline **GeoNames** `cities15000` dump downloaded once into `.cache/geonames/` (`--offline` never fetches: a cached dump is required, else `no-gazetteer`/exit 1). A place's `name` + `hierarchy` tokens match against the dump; country names and US state names narrow candidates (admin1/country codes), and **only a single high-confidence hit is proposed** - `ambiguous` (multi-candidate) and no-match places are skipped, never guessed. **Every write is gated by an interactive `[y/N]`** (`confirm` callable is injectable for tests). Writes are surgical text edits to the matched `places.yaml` block (coords inserted/replaced; `alt_names` added only when absent, never clobbering a human list), preserving comments without needing `ruamel.yaml`. The surgical edit ends a place's block at the next list item at the same-or-shallower indent as its `- id:` line (`_locate_place_block`, shared with set/note), so a uniformly indented registry (valid YAML) is edited correctly - the target entry alone changes and a later entry's `coords:` is never touched |
| `fha places set <L-id>` | ✓ 2026-07-17 | Human-directed registry edit: `--coords "LAT, LON"` (forgiving on shape, strict on ranges - ±90/±180, catching swapped pairs), repeatable `--aka NAME` (the complete also-known-as list, each name verbatim - commas and all, so "Washington, D.C." is one alias), repeatable `--history "PERIOD \| HIERARCHY"` (the complete names-over-time list). Each given field is a FULL replace of that one key; omitted fields untouched. Same one-block surgical write as geocode (comments and other entries preserved; multi-line values replaced whole via `_key_span`); `--dry-run` previews a unified diff. No `[y/N]` gate: unlike geocode these values are the human's own typing, and the command (or the workbench's preview→Apply) IS the confirmation the AGENTS.md coordinate rule requires. Exit 1 = unknown L-id (+ `fha find` next step); exit 3 = invalid id shape, out-of-range coords, nothing to change, unreadable/unwritable registry |
| `fha places note <L-id> --text TEXT` | ✓ 2026-07-17 | Appends a DATED research note to the place's `notes:` (reference prose, SPEC §15 - loose citations welcome). The existing value is preserved and the key is rewritten as a `notes: \|` block scalar (the one YAML shape that holds paragraphs legibly); the prior scalar style is read with plain `yaml.safe_load`, never guessed from text. Rendered on the place page (both site modes) now that the index carries `places.notes` |
| `fha places edit-note <L-id> --old-text TEXT --text TEXT` | ✓ 2026-07-17 | Rewrites ONE entry of the place's `notes:` append-log, named by its exact current text (same entry grammar as the person/source per-entry editors - `split_log_entries`); every other note stays as written. No match / ambiguous duplicate / empty replacement are plain refusals (removals stay a hand edit); the value re-emits as the same `notes: \|` block scalar the appender writes |
...
| Spacetime spans | ✓ | A sentence carrying exactly one `[S-id]` whose (subject, source) pair resolves to a single dated+placed claim **and** whose claim year appears in the sentence text is wrapped in `<span class="spacetime" data-loc=… data-date=ISO>`. **Accepted claims only** (owner decision 2026-07-22: a span stamps fact; a parked needs-review claim must not). The single-claim + year-in-sentence guards keep a source cited across several sentences from stamping the wrong (e.g. marriage) date onto an unrelated (e.g. birth) sentence. Sentence splitting skips initials ("Margaret A. Cole") and common abbreviations |
| Research Notes from parked claims | ✓ | The subject's needs-review claims export as open questions under `== Research Notes ==` ("Unconfirmed: {value} ({date}) - noted in \"{source}\". Not yet verified - corroboration welcome." - the date and source segments appear when present) so collaborators can pick them up - never as facts. Appends to the end of a profile-authored Research Notes section instead of duplicating the heading; a minted section is inserted before an authored `== Sources ==` so the trailing `<references/>` anchor stays attached to Sources. Withheld entirely: restricted claims, restricted/DNA/`publication_ok:false` sources, claims naming living/unknown persons, and claims naming a record-restricted person (deceased by-request included; unreadable person records fail closed) |
...
| Flag / feature | Status | Notes |
|---|---|---|
| `manifest.json` | ✓ M9.1 | One JSON object listing every operating-layer + skeleton file with `path`, `category` (`operating`/`skeleton`), `sha256`, `spec_version` (parsed from SPEC.md's version line), and a `src` field on skeleton entries whose archive path drops the `archive-template/` prefix. Built by a repo walk: the root rulebooks (`SPEC.md`/`TOOLING.md`/`AGENTS.md`/`CLAUDE.md`) and the two owner entry docs beside them (`GETTING_STARTED.md`/`CHEATSHEET.md` - repo path == install path, since nothing rewrites links at install time), the root launchers (`fha`/`fha.cmd`/`serve.cmd`), all of `tools/` and `design/` (minus `__pycache__`/`*.pyc`) - both remapped under the archive's `.fha/` via the `src`≠`path` seam - all of `docs/` (kept at the archive root), the agent workflow skills under `.claude/skills/`, and the `archive-template/` contents (minus its own `README.md`). Excludes the project `README.md` (repo-facing - badges, roadmap, contributing, and links to repo-only folders; an installed archive has no root README and starts at `GETTING_STARTED.md`), the tool-BUILDING docs (`BUILD*.md`, `TOOLING_INGESTION.md`, `TOOLING_INTERFACE.md`, `AGENTS_TOOLING.md`) - no tool reads them at run time and operating an archive never needs them - and spec-repo furniture (`example-archive/`, `archive-template/` as a folder, `tests/`, `.github/`, `.claude/settings.json`, `PRIVACY.md` - the public-repo "no real data" policy, contradictory inside a real archive - `RELEASE_CHECKLIST.md`, `manifest.json` itself) |
...
| Flag / feature | Status | Notes |
|---|---|---|
| `<P-id>` lookup | ✓ | file, couple folder, companions, claims, citations, photo note (the count names how many of those photos `reconcile` has flagged as no longer on disk, with `fha photoindex reconcile` as the fix) |
...
| `--text "…"` | ✓ | notes_fts + re.search; photo captions searched when photoindex is fresh, else explicit skip-note; transcript FTS populated + queried (CLI + JSON/workbench), shipped M11.5 (D7). A caption kept searchable after its file vanished (a `MISSING:` row) still matches, printed as the path it had plus `(not on disk)` rather than the internal key |
| `--related <P-id>` | ✓ | relationship edges (rel + distinct source count); co-occurring persons with no existing edge (per-tool duplicate of cooccur.py's person co-occurrence, scoped to one person); places by claim frequency; shared occupation/military/membership affiliations with other people; distinct source count; photos via `photo_people` (note if photoindex absent), each group located by a member still on disk - a group whose every member has vanished prints its last known path marked `(not on disk)` with the `reconcile` next step, never a `MISSING:` key |
| `--related <L-id>` | ✓ | claims naming the place; people ranked by claim frequency; distinct source count; micro-places (`within: L-id` children); photos within ~0.002° of the place's coords (same live-member-preferred locating rule as the person neighborhood) |
...
| `--json --kind K,K…` | ✓ | Narrows results to `person`, `source`, `place`, `hypothesis`, `claim`, `text` (comma list; unrecognised value refused with the valid list). Default (no `--kind`) is every kind. `photo-source` is an extra FILTER value (2026-07-17): only sources that actually own photo assets (`source_type: photo`, or any `source_files` path with a photo extension) - hits still carry type `source`. Built for the workbench's set-profile-photo picker; a pasted bare S-id still resolves under it (an explicit id is an explicit pick) |
...
| W103 | ✓ implemented | Stale couple-folder bracket lists, and a couple-folder name missing its `+ second spouse` half (add-only via the shared `_lib.spouse_extended_base` rule: fires only when the folder's derived couple has exactly two members, the base has no `+` yet, and the base exactly matches one partner's name - an existing `+ …` or hand-crafted base is never rewritten, and neither side may be a placeholder: a `004 unknown` base is left alone rather than fixed as `004 unknown + Jeanne`, which the add-only rule could never repair); fires in `fha lint` and `fha views brackets`. Marks a child who joined other than by birth (`Ruth (adopted)`, `(step)`, …) from the backing claim's `subtype` (SPEC §12.2). ⚑ The cross-link entry for a person who occupies multiple Ahnentafel positions (`Thomas Hartley (also #128 - see 040)`) is **deferred** - the lowest-numbered folder is already the deterministic home (BFS-first), so this is a display-only follow-up. |
...
| W119 | ✓ implemented | Direct-line ancestor (derived Ahnentafel position ≥ 2) still filed as a stub (`tier: stub` or record under people/stubs/); fires warning-severity in `fha lint` (report-only, lead-worded - the fix named is `fha views brackets --fix-promote` or `fha person promote`) and as check 4 in `fha views brackets` (requires `root_person`; `--generations` caps depth). Surfaced as `fha report` §7b. |
| W120 | ✓ implemented | Ahnentafel slot defaulted, not derived (requires `root_person`): a couple with exactly one resolved parent whose `sex:` is unrecorded (absent, blank, the legacy `U`, or a value outside the M / F / intersex / unknown vocabulary - an explicitly recorded `intersex`/`unknown` is a stated fact and stays silent) takes the father/even slot by default, so the derived folder numbers above them look confirmed while being a guess W110 cannot dispute. Fires report-only in both `fha lint` and `fha views brackets`; the named fix is recording `sex: M`/`sex: F`, then re-running the check. Silent for two resolved parents with unset/matching sex (the genuine same-sex/unknown tie-break case, TOOLING §7). |
| W121 | ✓ implemented | `roots:` change orphaned filed assets: a `roots:` value differs from the one the tools last ran against (`.cache/roots.json`, seeded by install/update-tools and advanced by `fha index`/`fha doctor`; lint reads it but never writes), and source `files:` entries under that alias that resolved under the old value no longer do. One finding on `fha.yaml` ahead of the E011 fallout, naming old/new values, count and sample; sticky until reverted or re-pointed. Points at `photos_ignore:` as the right tool for keeping material out of the photo catalog. |
| W122 | ✓ implemented | Person filename and person record disagree: a file under `people/` whose name ends in a companion-kind word (`…_timeline_P-…`, `…_research_P-…`) while its own frontmatter carries the SPEC §9 person fields (`name:` / `living:`). SPEC §13's kind slot and the last given-name segment are one slot, so the grammar cannot tell a generated timeline from the record of a woman named Marie Timeline; content decides (`_lib.person_file_kind` - the shared rule `fha index` reads too), and this reports the contradiction instead of resolving it in silence. Report-only, reassuring in tone, and no fix flag: the message says the record is read correctly, names the shortened filename that ends the ambiguity (the slug never has to carry every given name), and says keeping the name is fine and the note will keep appearing. Silent for a genuine generated companion and for an ordinary profile. |
...
| `--fix-inventory` | ⚑ superseded (deliberately not re-added) | The real E011 fixer shipped as the standalone `fha reconcile` (reconcile.py, TOOLING §9, BUILD M11.1); E011's message names it. A lint fix-flag alias would be a second pathway for the same mutation - one owner per side effect. |
