...
| `fha views brackets` | `views.py` | ✓ W103 bracket + `+ spouse` name refresh, W110 Ahnentafel placement, W119 direct-line stubs, W120 defaulted-slot note; `--fix` applies W103/W110, `--fix-promote` applies W119 under the same Apply? gate (refused together), `--realign` applies both halves in one previewed pass (destinations recomputed against the post-rename tree), `--generations N` caps W119 depth, `--dry-run` previews |
...
| W120 | ✓ implemented | Ahnentafel slot defaulted, not derived (requires `root_person`): a couple with exactly one resolved parent whose `sex:` is unrecorded takes the father/even slot by default, so the derived folder numbers above them look confirmed while being a guess W110 cannot dispute. Fires report-only in both `fha lint` and `fha views brackets`; the named fix is recording `sex: M`/`sex: F`, then re-running the check. Silent for two resolved parents with unset/matching sex (the genuine same-sex/unknown tie-break case, TOOLING §7). |
