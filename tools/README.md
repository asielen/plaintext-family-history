...
| `--fix-ids` | ✓ implemented | Mints IDs for hand-authored, id-less records (SPEC §10 mint-on-contact): renames to the §13 grammar and keeps BOTH the slug and the verbatim filename stem as aliases so existing `[[name]]` links keep resolving. Never touches a GENERATED-headed file or a `README.md` (excluded from the auto-mintable set entirely). Second half: mints missing claim `id:`s by surgical text insertion (every other byte preserved), and stamps `reviewed:` (today) on a hand-written `accepted` claim it is minting an id into - the hand-written `accepted` is the human's decision already on record; the stamp records when the tools met it (TOOLING §3b: the editing method doesn't matter, only that the decision is theirs). Accepted claims that already carry ids are untouched (E006 still owns those). Refuses one-line `- {...}` flow items it cannot align safely, naming `fha id mint C` as the by-hand fix. All previewed by `--dry-run` |
