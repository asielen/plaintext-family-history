...
def _tables_with_path_column(conn: sqlite3.Connection) -> list[str]:
    """Every table (FTS included) carrying a `path` column - the relocation set."""
    names = [
        row[0] for row in conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' "
            "AND name NOT LIKE 'sqlite_%' AND name NOT LIKE '%\\_config' ESCAPE '\\' "
            "AND name NOT LIKE '%\\_data' ESCAPE '\\' AND name NOT LIKE '%\\_idx' ESCAPE '\\' "
            "AND name NOT LIKE '%\\_docsize' ESCAPE '\\' AND name NOT LIKE '%\\_content' ESCAPE '\\'"
        )
    ]
    out = []
    for name in names:
...
def relocate_person(
    ...
    *,
    ...
    """
    Keep the index correct across a person-record move without a rebuild (#37).
    ...
    the persons row, and a freshly scaffolded research companion is indexed
    like any other person file. The sqlite write also bumps index.sqlite's
    own mtime past the record's, so the freshness check reads 'fresh' -
    correctly, because it is.

    Returns 'indexed', or 'index_absent' when there is no full index to
    update (promote then just says 'run fha index', as before).
    """
    cache_dir = archive_root / '.cache'
    if not _require_existing_index(cache_dir):
        return 'index_absent'
    ...
    try:
        with conn:
            tables = _tables_with_path_column(conn)
            ...
            for path in new_files or []:
                _index_person(conn, Path(path), archive_root)
    finally:
        conn.close()
    return 'indexed'
