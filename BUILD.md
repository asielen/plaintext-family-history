...
**One PR.** New file `tools/index.py`. Wire `fha index [--source S-id] [--root PATH]`
into `fha.py` (TOOLING §2).
...
5. Build FTS tables.
6. Derive `relationships` edges from accepted claims (see below).
...
**Relationship derivation** (after claims load): for each `accepted` claim —
`relationship subtype: child-of` → `(child, 'parent', father)` + reciprocal; `marriage` or
`relationship subtype: spouse-of` → reciprocal `spouse` edges with `date_start`/`date_end`;
social subtypes → `friend`/`associate`/`neighbor` edges. Edges are pure cache.
...
**One PR.** New file `tools/lint.py`. Wire `fha lint [--root PATH] [--json]
[--with-exif] [--dry-run] [--mint-stubs] [--spawn-questions] [--fix-inventory]
[--format-check] [--format-write]` into `fha.py` (TOOLING §3).
...
# repeat for each E/W code that has a broken fixture
...
## Layer 2 — Archive views & discovery

Depends on: index.

---

### `fha views` — timeline, sources-index, draft-queue
...
### `fha views` — brackets, tree, clean, refresh
...
### `fha find` — ID types, `--text`, `--related` stub
...
### `fha photoindex` — scan, schema, variation grouping, `find` sub-command

**One PR.** New file `tools/photoindex.py`. Wire `fha photoindex [--full]` and
`fha photoindex find` into `fha.py` (TOOLING §9).
...
- Face regions: parse `XMP-mwg-rs:RegionInfoMP` → `{name, type, area}` list.
- `photo_people` in confidence order: `pid-keyword` (bare `P-id` keyword, authoritative) →
  `face-tag` (region name matched exactly against `person_face_tags.tag`; ambiguous = skip) →
  `name-match` (persons.name; weakest, flag in output).
...
`_run_exiftool()` so CI can inject pre-cooked JSON. Include at least one variation pair and
one image with a `SOURCE:` keyword.
...
### `fha photoindex` sub-commands — triage, reconcile, tag-person, report; unlock D7

**One PR.** Extend `tools/photoindex.py`. Also update `tools/find.py` to include `photo_fts`
in `--text` searches (D7 unlock) (TOOLING §9, §15b).
...
## Layer 4 — Cross-reference & connection
...
### `fha xref` + `fha cooccur`

**One PR.** New files `tools/xref.py` and `tools/cooccur.py`. Wire into `fha.py`.
Neither writes to the archive — output candidates only. Require fresh index; exit 3 if absent
(TOOLING §14a, §14a2).
...
### `fha find --related` — complete implementation
...
### `fha report` — sections 5–8

**One PR.** Extend `tools/report.py` (TOOLING §15a).
...
### `fha places lint` + `fha places candidates`
...
## Layer 7 — Intake pipeline

`fha process` depends on: index, lint. `fha capture` hands off to process.
`fha convert-mining` depends on: index, lint, stubs. Split `fha process` into three PRs —
variation detection + bundle dissolution is significant scope on its own.

---

### `fha process` — document files (Stage A, single-file mode)
...
### `fha process` — folder mode, variation detection, bundle dissolution
...
**Bundle folder dissolution** (SPEC §12.1). Folder with `notes.md` = source stub bundle.
...
### `fha capture` — paste fallback + generic recipe
...
### `fha capture` — site recipes

**One PR.** Create `tools/capture_recipes/` directory. Each recipe exposes
`detect(html, url) -> bool` and `extract(html, url) -> dict`. `fha capture` tries recipes in
priority order: Ancestry → FamilySearch → Newspapers.com → FindAGrave → generic fallback.
...
table); Newspapers.com (publication, date, page, article snippet, formatted citation);
FindAGrave (memorial name, birth/death, cemetery as `place_text`, persons).
...
# each recipe: detect() True for own sample, False for others'
```
...
contract). Jinja2 is the one new permitted dependency; add to requirements. Three PRs —
each is independently shippable.

---

### `fha site` — foundations: query layer, Jinja2, person + source pages
...
`.md` view files. Biography prose and Stories sections come from the curated person `.md` body.

`tools/templates/` directory with `base.html` layout. Token swap: `TOKEN_RE` → relative hrefs;
unresolved tokens → `<mark>[X-xxxx]</mark>`. Minimal stdlib HTML converter for profile prose
(headings, bold, lists, links) — no markdown library.

`--standalone` (default): web-optimized derivatives (max 1200px, EXIF stripped) into
`site/media/`; redaction baked in (living/unknown → "Living Person", no person page; restricted
/ DNA / `publication_ok: false` sources excluded). `--linked`: fast local preview; real archive
paths; no copies; no redaction.

**Person page (curated):** summary block (accepted vitals); biography HTML; timeline (same
index query as `fha views timeline`); sources index (same query as `fha views sources-index`);
photo strip (from `photo_people`); Stories; Friends & Family (relationship edges).
...
### `fha site` — home, place, discoveries pages + standalone redaction
...
Verify `--standalone` redaction covers all page types — place pages must not link to redacted
persons; discoveries page must redact living-person names.

**Done when:**
```sh
fha site --root example-archive --linked    # home, place, discoveries pages present
...
### `fha site` — interactive tree rendering
...
### `fha install` + `fha update-tools`

**One PR.** New file `tools/scaffold.py`. Wire both commands. Create `manifest.json` at repo
root (TOOLING §13c).
